...
We use the `lender loans` and `loans` files for this analysis. We import these files using the `read_csv` function. The `lender loans` file contains detailed loans and lender information with corresponding borrowers. The `loans` file has detailed borrower information and their respective loans, however this file does not contain any lender information. 

Our prompt focuses on motivations for lending to developing-nation entrepreneurs, therefore to incorporate development indicator we use data from world bank which helps to separate the borrowers' countries into low income and high income groups in the `lender loans` file. We perform this step using Microsoft excel and import the updated data set for the rest of our analysis

```{r input_data, echo=TRUE, warning=FALSE, results='asis'}

#kiva_ll <- read_csv("data/lender_loans.csv")
kiva_ll <- read_csv("data/lender_loans_with_wbi.csv") #lender loans updated with word bank indicators
...
We begin by getting an overview of the characteristics of all would-be borrowers, including their gender, sector, country, and requested loan amount. First we look at how the loan amount varies between high income and low income countries. From the bar chart below, we can observe that countries belonging to lower income group have received higher loans in terms of amount.
...
Keeping our prompt in mind, we subset our data to these low income countries and perform the rest of the analysis on that subset. Because we will ultimately be interested in differences between funded and non-funded borrowers, we add an indicator for borrower type and funding status to our data set. This will help improve our visualizations and communicate trends at these two levels. 
...
We look at the distribution of the funding status of loans among borrower types for various sectors.
<!-- We will visualize this data in a tabular format since our data contains two categorical variables. -->
