...
Interpreting the coefficients in a logistic regression is a little less straight-forward than in linear regression, but still quite simple. For example, the coefficient for `posted_yr` is "0.44", which means that for a given year when the loan is posted, the expected odds of the loan getting repaid is $e^{0.44}\approx1.55$ times the odds of the loan getting repaid if the loan is posted in the previous year. 

Among all sectors, a loan in the Education sector has the highest odds of getting repaid, while loans in the Personal Use sector are the least likely to be repaid. Loans in the Housing sector has $\frac{e^0.63}{e^0.52} \approx 1.116$ times the odds for loans in the Food sector to be repaid. In terms of the continent where the loan transaction took place, besides Oceania which has highest coefficient but is not statistically significant, Europe is the continent where loans generally have the highest odds of being repaid ($e^3.47-e^1.51\approx 7.1$ times the odds for Asia, 15.33 times the odds for Americas, and 32.14 times the odds for Africa). As for the sex of the borrower(s), a female borrower or a female-only borrower group has the odds of repaying the loan  $1/e^{-0.22}\approx 1.24$ times that for a male borrower or a male-only borrower group.
...
Here we got an AUC score of 0.7094, which means there is about 70% chance that our model will be able to distinguish between positive class (paid) and negative class (defaulted). In general, an AUC of 0.5 suggests no discrimination, 0.7 to 0.8 is considered acceptable, 0.8 to 0.9 is considered excellent, and more than 0.9 is considered outstanding.
...
Since difference is positive, the expected predictive accuracy for the second model (post) is higher.
...
We got an AUC score of 0.7123, which is slightly higher than the one for the frequentist model (0.7094).
