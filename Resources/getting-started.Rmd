...
Before we begin, an important caveat to note is that the 2012 DataFest dataset is not an exhaustive record of all Kiva borrowers, but a subset selected for the purposes of the competition. In order to proceed with our analysis, we must assume that the data provided is a representative sample. However, note that if this assumption is violated, i.e., if the data differs systematically from the full population of Kiva transactions due to seleciton bias, our inferences will not be valid.
...
As an example, the `kiva_loans` dataframe includes information on loan amounts and years. We can use this to see which years have the highest loan amounts as well as how loan amounts change over time. Were there specific years that loan amounts dipped or increased, and can we use that information to draw conclusions about how people repay or give out loans? We also have geographical information for the loans. This can be used to create a model to see where loans are coming from and where they are given to. Using this information, we can examine lender motivations: Do people prefer lending to borrowers from the same country, different countries, or is there no correlation? Take some time to think about the relationships you can observe and ask yourself which ones are the most relevant for the questions you want to answer. There is a multitude of angles and approaches to take when examining complex datasets, so it's important to narrow down your strategy to fit within the time restrictions of DataFest.
...
* both teams work on preliminary model fitting and ealuation

2:00 PM - 6:00 PM

* both teams work on advanced model fitting and evaluation
