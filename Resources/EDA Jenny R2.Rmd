...
# Summarize the loan amount based on the lenders' country
...
country_decoded <- country %>%
  ...
  filter(country_name %in% c("US", "CA", "GB", "AU", "DE", "NL", "SE", "NO", "JP", "PH"), year < 2012) %>%
...
top_lender_borrow <- aggregate(top_lenders$loan_amount, by = list(lender_country = top_lenders$country_code, borrower_country <- top_lenders$location.country), FUN = sum, na.rm = TRUE) %>%
  ...
  select(!c(`Borrower Country`, `Loan Amount (USD)`)) %>%
  distinct()
...
# Calculate the unpaid loan amount
