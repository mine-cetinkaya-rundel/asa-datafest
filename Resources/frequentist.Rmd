...
The logistic regression model is an example of a broad class of models known as generalized linear models (GLM), in which a linear relationship between the response and predictors is built even though their underlying relationship is not linear. Since we have a single-observation binary response variable, we are using a Bernoulli GLM. A Bernoulli distributions the discrete probability distribution of a random variable which takes the value 1 with probability $p$ and the value 0 with probability $q=1-p$.The likelihood for one observation $y$ can be written as a Bernoulli PMF over possible outcomes $k$: $$\begin{equation}
    f(k;p) =
    \left\{
        \begin{array}{cc}
                p & \mathrm{if\ } k=1 \\
...
-   `loan_amount`, because we think that the smaller loans tend to be more easily paid back then the larger ones;
...
-   `time`, which is a variable we created representing the time from when the loan is posted to when the loan is funded, in months, since sometimes the time it take for the borrowers to receive the loan may affect their financial situation and thus affect their ability to repay the loan according to **t**he terms agreed.
...
-   `sex`, which indicates the sex of a borrower group, where "mixed" means in a borrower group there are both males and females.
...
However, a closer look at the variable `dif` showed that after we filtered the original data set to contain only completed loans, all the observations has funded amount equal to the loan amount. Thus, the variable `dif` would not make any difference on the response variable, so we were excluding it from our model.
...
From the p-values of model coefficients, the significant predictors are `loan_amount`, `sector`, `posted_yr`, and `continent`, which have p-values less than 0.05. `sex` can also be considered significant if we choose the significance level $\alpha=0.1$ . Here we can notice that the coefficient of `loan_amount` is about 0, meaning that the odds of loan repayment is expected to be almost the same across all values of loan amount, which indicates the loan amount has little effect on whether the loan is paid back or not. Also, the p-value is extremely close to 0, indicating a strong statistical significance of this predictor. Another finding was that the loans in the "Education" sector is the most likely to be repaid, while loans in the "Personal Use" sector is the least likely to be repaid. In terms of the continent where the loan transaction took place, Oceania has the highest odd ratio for the loan being repaid while Africa has the lowest odd ratio. A borrower group that consists of both males and females has the highest odd ratio of repaying the loan, and a female borrower or a female-only borrower group generally has higher odd ratio of repaying the loan than a male borrower or a male-only borrower group.

An example of interpretation of the coefficients is: "0.42" for `posted_yr` means that for a given year when the loan is posted, the expected odds of the loan getting repaid is $e^{0.42}\approx1.52$ times the odds of the loan getting repaid if the loan is posted the previous year.

To evaluate our model, we're using the ROC (*Receiver Operating Characteristic*) curves to evaluate different thresholds for a classification problem. In a nutshell, ROC curve visualizes a confusion matrix for every threshold. ROC curve shows a *False positive rate* on the X-axis. This metric informs us about the proportion of negative class classified as positive. We used `ggroc()` from the `PROC` package to plot the ROC curve.
