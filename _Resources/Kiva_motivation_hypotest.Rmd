...
As you recall form your Introduction to Statistics class, the p-value (probability value) is the level of significance within a statistical hypothesis test, representing the probability of occurrence of a given event. If the p-value is greater than our level of significance, we accept the null hypothesis, whereas if the p-value is lesser than our level of significance, we reject the null hypothesis. This is also referred to as the decision rule. For our analysis we will set the significance level at 0.001 or 0.1%.
...
The one-sided t-test results in a p-value greater than the level of significance 0.001 so we fail to reject the null hypothesis. This indicates that there is no difference in the sample means of average loan amount of male and female borrowers. We could interpret this as evidence that the gender of a borrower is not a source of motivation for lenders, however we must note that this does contradict what we have observed during EDA.
